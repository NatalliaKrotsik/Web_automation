# Beginner's Tutorial — PrettyBank Test Automation Framework

Welcome! This guide will take you from zero to writing and running your first API and UI tests
in the PrettyBank automation framework. Read every section in order — each one builds on the previous.
...
This is a **Python test automation framework** for PrettyBank — an internal banking web application.
...
print(config.base_url)       # https://dev.pretty-py.andersenlab.dev
print(config.api.base_url)   # https://api-dev.pretty-py.andersenlab.dev
...
All commands must be run from the project root folder (`pretty-autotests/`).
...
```python
# WRONG ❌
page.goto("https://dev.pretty-py.andersenlab.dev")
