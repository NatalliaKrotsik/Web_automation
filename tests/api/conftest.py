...
def auth_tokens(load_environment) -> dict:
    ...
    tokens = auth_api.authorization(
        email=EnvManager.get("USER_EMAIL_DEV"), password=EnvManager.get("USER_PASSWORD_DEV")
    )
    return tokens.json()
...
def admin_auth_tokens() -> dict:
    ...
    auth_api = AuthAPI()
    tokens = auth_api.authorization(
        email=EnvManager.get("ADMIN_USER_EMAIL"), password=EnvManager.get("ADMIN_USER_PASSWORD")
    )
    return tokens.json()
